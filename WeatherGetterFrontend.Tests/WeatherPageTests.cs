namespace WeatherGetterFrontend.Tests
{
    ...
    {
        [Fact]
        public void Weather_WhenNoCitySelected_ShouldNotInvokeChartJs()
        {
            ...
            JSInterop.SetupVoid("weatherChart.render", _ => true);
            ...
            JSInterop.Invocations.Count(x => x.Identifier == "weatherChart.render").Should().Be(0);
        }

        [Fact]
        public async Task Weather_WithSelectedCity_ShouldInvokeChartJs()
        {
            var mockHttp = new MockHttpMessageHandler();
            mockHttp.When(HttpMethod.Get, "http://localhost/weather/Get-AllCities")
                .Respond("application/json", "[{\"id\":1,\"city\":\"Warsaw\",\"latitude\":52.23,\"longitude\":21.01}]");
            ...
            var selectedCityField = typeof(Weather).GetField("selectedCityName", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            {
                var renderChartField = typeof(Weather).GetField("renderChart", BindingFlags.Instance | BindingFlags.NonPublic);
